...
public class CowboyBehavior : MonoBehaviour
{
    [Header(" --- References --- ")]
    ...
    [SerializeField] float m_ReloadSpeed = 1f;
    [SerializeField] float m_FireDistance = 6f;
    [SerializeField] float m_StopDistance = 3f;
    ...
    // Rotates the cannon so it always points at the player.
    void AimAtPlayer()
    {
    ...
    // Fires a bullet toward the player and waits for the reload duration.
    IEnumerator FireCoroutine()
    {
        m_IsFiring = true;
        animator.SetBool("isFiring", true); // Start shooting animation
        PlayAudio();
        ...
        animator.SetBool("isFiring", false); // Stop shooting animation
        m_IsFiring = false; // Allow firing again
    }
